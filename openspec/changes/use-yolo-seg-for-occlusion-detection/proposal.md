...
- 保留 OpenCV 后端作为可配置回退能力，便于权重缺失或运行环境缺少 YOLO 依赖时排查问题。
...
- 配置：更新 `[occlusion_detection]`，默认 `backend` 切换为 `yolo_seg`，权重路径改为 `model/best.pt`，新增或明确 `threshold` 与 `area_ratio` 默认值。
- 代码：
